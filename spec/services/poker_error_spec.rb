...
RSpec.describe PokerError do
  describe 'エラー判定' do
    ...
    context 'カードは5枚だがスート全てに不正文字が含まれているケース' do
      ...
      it { is_expected.to eq ["1番目のカード指定文字が不正です。（Q1）", "2番目のカード指定文字が不正です。（@2）", "3番目のカード指定文字が不正です。（s3）", "4番目のカード指定文字が不正です。（04）", "5番目のカード指定文字が不正です。（#5）"] }
    end
    ...
    context 'カードは5枚だがスート全てに不正文字が含まれているケース' do
      ...
      it { is_expected.to eq ["1番目のカード指定文字が不正です。（S0）", "2番目のカード指定文字が不正です。（S２）", "3番目のカード指定文字が不正です。（S14）", "4番目のカード指定文字が不正です。（S04）", "5番目のカード指定文字が不正です。（S5-2）"] }
    end
